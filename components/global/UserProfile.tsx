...
export default function UserProfile() {
  ...
  // --- 1. SKELETON STATE ---
  // This replaces the "if (!session) return null" or loader.
  ...
  if (!session?.user) {
    return (
      <a href="/api/auth/signin" className="px-5 py-2 bg-primary text-white rounded-xl font-bold text-sm hover:opacity-90 transition-all shadow-md shadow-primary/20">
        Sign In
      </a>
    )
  }
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 p-1.5 pr-3 hover:bg-gray-100 dark:hover:bg-neutral-800 rounded-2xl transition-all group border border-transparent hover:border-gray-200 dark:hover:border-neutral-700"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 mt-3 w-64 bg-white dark:bg-neutral-900 rounded-2xl shadow-xl border border-gray-100 dark:border-neutral-800 py-2 z-[110] animate-in fade-in zoom-in-95 duration-200">
          <div className="px-4 py-4 border-b border-gray-50 dark:border-neutral-800 flex items-center gap-3">
             <div className="w-10 h-10 rounded-xl relative overflow-hidden bg-primary/10 flex items-center justify-center text-primary font-bold">
                {displayImage ? <Image src={displayImage} alt="Avatar" fill className="object-cover" /> : userInitials}
             </div>
             <div className="flex-1 truncate">
                <p className="text-sm font-bold text-neutral-900 dark:text-white truncate">{userName}</p>
          ...
          <div className="px-2 pt-2 border-gray-50 dark:border-neutral-800">
            <button
              onClick={() => signOut()}
              className="flex items-center gap-3 px-3 py-2.5 text-sm font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-xl transition-colors w-full"
            >
